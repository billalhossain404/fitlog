...
const IndiCard = ({ card }: ICardProps) => {
  return (
    <Link href={`/WorkOuts/${card.id}`}>
      <div className="bg-[#15171D] border border-[#292C33] rounded-2xl overflow-hidden transition duration-300 hover:-translate-y-2 hover:shadow-lg cursor-pointer">
        <Image
          src={card.image}
          alt={card.name}
          width={500}
        ...
        <div className="p-6">
          <div className="flex flex-wrap gap-2 mb-4">
            {card.muscleGroups.map((muscle: string, index: number) => (
              <span
                key={index}
                className="bg-[#C7FF00] text-black px-3 py-1 rounded-full text-xs font-bold uppercase"
              >
                {muscle}
              </span>
            ))}
          ...
          <p className="text-gray-400 mt-2">{card.equipment}</p>
